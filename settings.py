...
REQUESTED_APP_BASE_PATH = normalize_base_path(
...
APP_BASE_PATH: str = (
    ...
    if (
        not DEBUG
        or env_bool('ENABLE_APP_BASE_PATH_IN_DEBUG', False)
    )
    else ''
)
...
if (
    os.environ.get('SECURE_SSL_REDIRECT') is None
    and any(arg.endswith('runserver') for arg in sys.argv)
):
    default_secure_ssl_redirect = False

SECURE_SSL_REDIRECT = env_bool(
    'SECURE_SSL_REDIRECT',
    default_secure_ssl_redirect
)
...
if DATABASE_URL:
    database_url_is_sqlite = DATABASE_URL.lower().startswith(
        'sqlite'
    )
    ...
    DATABASES = {
        'default': dj_database_url.parse(
            ...
            ssl_require=env_bool(
                'DB_SSL_REQUIRE',
                not DEBUG and not database_url_is_sqlite
            ),
        )
    }
    ...
    if os.environ.get('DB_UNIX_SOCKET'):
        mysql_options['unix_socket'] = os.environ[
            'DB_UNIX_SOCKET'
        ]
...
#
# IMPORTANT:
# Do NOT define DEFAULT_FILE_STORAGE here.
# Django's modern STORAGES configuration is used instead.
#
...
if USE_S3_STORAGE:
    ...
    if missing_s3_settings:
        raise ImproperlyConfigured(
            'USE_S3_STORAGE=true requires these '
            'environment variables: '
            f'{", ".join(missing_s3_settings)}.'
        )
...
WHATSAPP_PHONE_ID = os.environ.get(
    'WHATSAPP_PHONE_ID',
    ''
)
...
WATI_API_URL = os.environ.get(
    'WATI_API_URL',
    ''
).rstrip('/')

WATI_ACCESS_TOKEN = os.environ.get(
    'WATI_ACCESS_TOKEN',
    ''
)

WATI_INSTANCE_ID = os.environ.get(
    'WATI_INSTANCE_ID',
    ''
)

DEFAULT_WHATSAPP_COUNTRY_CODE = os.environ.get(
    'DEFAULT_WHATSAPP_COUNTRY_CODE',
    '91'
)

BILL_WHATSAPP_API_ENABLED = env_bool(
    'BILL_WHATSAPP_API_ENABLED',
    False
)

WHATSCHIMP_ENABLED = env_bool(
    'WHATSCHIMP_ENABLED',
    False
)

LEAD_CAPTURE_API_KEY = os.environ.get(
...
DEFAULT_FROM_EMAIL = os.environ.get(
    'DEFAULT_FROM_EMAIL',
    EMAIL_HOST_USER or 'noreply@yourcrm.com'
)
...
if HAS_DRF_SPECTACULAR:
    ...
    SPECTACULAR_SETTINGS: dict = {
        'TITLE': 'CRM ERP API',
        'DESCRIPTION':
            'Complete CRM + ERP backend for institute management',
        'VERSION': '1.0.0',
